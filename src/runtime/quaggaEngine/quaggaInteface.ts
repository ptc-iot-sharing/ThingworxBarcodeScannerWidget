...
export class QuaggaInstance {
        ...
        this.config = {
            inputStream: {
                ...
                constraints: {
                    ...
                    // extract the height and width from the resolution
    ...
    decodeSingleImage(image: string): void {
        const singleConfig = Object.assign({}, this.config);
    ...
    /**
     * Sets the torch to the specified value. Will only work on compatible devices
     * @param value New value for the torch
     */
    setTorch(value: boolean): Promise<void> {
        const track = Quagga.CameraAccess.getActiveTrack();
        if (track && typeof track.getCapabilities === 'function') {
            // check disabled because torch is an non-standard property
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            return (track as any).applyConstraints({ advanced: [{ torch: !!value }] });
        }
    }
    ...
    setZoom(value: number): Promise<void> {
        const track = Quagga.CameraAccess.getActiveTrack();
        if (track && typeof track.getCapabilities === 'function') {
            // check disabled because zoom is an non-standard property
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            return (track as any).applyConstraints({ advanced: [{ zoom: value }] });
        }
    }
     ...
     */
    removeDetectionListener(listener: DetectionListener): void {
